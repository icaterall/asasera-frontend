...
### Activity sharing and feedback extension — 9 September 2026

The [community surface record](.impeccable/surfaces/community.md) documents `/activities/:id`, `/teacher/feedback`, teacher-only recommendations and their sidebar, owned-row, editor and discovery entry points. This extension retains the current Asasera palette, bilingual typography, shared controls and existing activity-world thumbnails. Public pages expose published previews and numeric rating totals; written teacher feedback and creator replies remain private. Source-defined layouts, local type/corner values, breakpoints, states and artwork provenance stay in that scoped record and do not change the global token scales.

[Verification](.impeccable/review/community/verification.md) records passing functional checks and the single detector run's 15 advisory local type/radius differences, with no non-advisory findings. The [finish review](.impeccable/review/community/finish-review.md) returns **recapture**: current 1440px/390px English/LTR and Arabic/RTL light/dark captures are absent because of the existing administrator-enforced browser restriction. Rendered appearance, responsive layout, overflow and real-browser keyboard/focus behavior remain unverified; a full finish re-review is required. This is a source record, not visual approval.
...
### Activity and course audience

Audience forms use the database's categories (one required choice), education stages (one or more) and countries (optional, multiple). Both creation and editing use `AudienceFields`; country chips can be removed individually or cleared for any country. Existing saved country selections always win over a new IP suggestion. A new form suggests the IP country once. Arab visitors see their suggested country, then the other Arab countries, then all remaining countries, alphabetized in the interface language.

`MultiSelect` extends the shared dropdown treatment with search, checkboxes, removable selections, Clear selections and Done. It retains the existing Montserrat/Arabic fonts, blue accent, 48px trigger, 44px minimum row and action targets, 8px chips, 7px action corners and 12px popup radius. Those choices intentionally match the established dropdown component. Country/stage search accepts either language. Inline errors preserve choices; a country lookup failure does not prevent saving for any country. The activity editor offers an inline Category & audience disclosure; courses expose Edit details & audience, and inline course creation in a new lesson uses the same fields.


### Answer rectangles — 11 September 2026

Correct-answer radios sit inside the answer rectangles and render as circular green checks. Empty options keep a white surface with their colored shape strip; their correctness control is disabled and hidden until text or an approved image exists. Each rectangle ends with an image action, independent of the focus-only formatting toolbar. Upload progress remains directly below the relevant answer, with the image action disabled during upload and checking. Images may be the entire answer. Keyboard radio behavior, 44px controls, 5px corners, English desktop and Arabic mobile layouts were checked. Publication accepts image-only options but rejects options with neither text nor image; the backend update joins the pending coordinated media rollout.
