...
test('Express legal pages remain readable in both languages with JavaScript disabled',async({page})=>{
 ...
 for(const path of ['privacy','data-deletion']){
  const response=await page.goto(`http://127.0.0.1:4100/${path}`);expect(response?.status()).toBe(200)
 ...
 const missing=await page.request.get('http://127.0.0.1:4100/api/v1/unknown-review-route');expect(missing.status()).toBe(404);expect(missing.headers()['content-type']).toContain('application/json')
