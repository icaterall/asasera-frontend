...
import BillingPlans from '@/features/account/BillingPlans'
import type {PublicUser} from '@/lib/api'
import en from '@/i18n/locales/en'
import ar from '@/i18n/locales/ar'

// Synthetic account. The browser tests intercept all network requests; no payment.
...
document.documentElement.dir=arabic?'rtl':'ltr';document.documentElement.lang=arabic?'ar':'en'
localStorage.setItem('asasera.theme',params.get('theme')||'light')
const user={id:73,name:arabic?'أحمد الراشدي':'Ahmed Al Rashdi',email:'instructor@example.test',role:'teacher',emailVerified:true,locale:arabic?'ar':'en'} as PublicUser
createRoot(document.getElementById('root')!).render(<I18nextProvider i18n={i18n}><ThemeProvider><QueryClientProvider client={new QueryClient()}><AuthContext.Provider value={{status:'authenticated',user,accessToken:null,needsProfile:false,signIn:async()=>user,signOut:async()=>{},forgetSession:()=>{},applyUser:()=>{}}}><MemoryRouter initialEntries={['/teacher/billing?checkout=done&session=cs_test_fixture123']}>
 <TeacherHeader onOpenNav={()=>{}}/><main><BillingPlans/></main>
