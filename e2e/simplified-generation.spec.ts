...
async function paidFixtures(page:Page,{locale='en',unavailable=false}:{locale?:string;unavailable?:boolean}={}){
 ...
 await page.route('**/api/v1/activity-generation/**',async route=>{
  ...
  if(path.endsWith('/quote'))return route.fulfill({json:{quoteId:'11111111-1111-4111-8111-111111111111',quoteExpiresAt:'2999-01-01T00:00:00Z',estimateMillicents:1,maxAuthorizedMillicents:149,estimateAiCredits:1,maxAuthorizedAiCredits:149,usableAiCredits:99999,creditPolicyVersion:1,creditUnit:'AI Credits',spendableMillicents:99999,usableMillicents:99999,allowanceMillicents:99999,exposureMillicents:0,affordable:true,pricingAvailable:true,generationAvailable:!unavailable,grant:{trialMillicents:99999,trialAiCredits:99999,claimed:true,eligible:true,reason:null},delivery:'5 questions'}})
...
for(const m of [{locale:'en',w:1440,h:900,dark:false},{locale:'ar',w:1440,h:900,dark:true},{locale:'ar',w:768,h:1024,dark:false},{locale:'ar',w:390,h:844,dark:false},{locale:'en',w:360,h:740,dark:true}]){
 test(`compact creation/review browser fixture ${m.locale} ${m.w} ${m.dark?'dark':'light'}`,async({page})=>{
  ...
  await create.click()
  const confirm=dialog.getByRole('button',{name:m.locale==='ar'?'تأكيد وإنشاء الأسئلة':'Confirm and create',exact:true});await expect(confirm).toBeEnabled()
  const credits=(await dialog.locator('footer bdi').allTextContents()).map(v=>Number(v.replace(/[٠-٩]/g,c=>String('٠١٢٣٤٥٦٧٨٩'.indexOf(c))).replace('٫','.').replace(/[^0-9.]/g,'')));expect(credits).toEqual(expect.arrayContaining([1,149,99999]));await expect(dialog.locator('footer')).not.toContainText('$')
  expect(await dialog.evaluate(el=>el.scrollWidth<=el.clientWidth)).toBe(true)
  const footer=await confirm.boundingBox();expect(footer!.y+footer!.height).toBeLessThanOrEqual(m.h)
  ...
  await confirm.click();await expect(page.getByText(m.locale==='ar'?'جارٍ تحضير أسئلتك…':'Preparing your questions…',{exact:true})).toBeVisible()
  ...
  await page.keyboard.press('Escape');await expect(dialog).not.toBeVisible();m.w>=700?await expect(opener).toBeFocused():await expect(opener).toBeVisible()
 })
}
test('quote changes, source switching and unavailable recovery browser fixture',async({page})=>{
 ...
 await dialog.getByRole('button',{name:'Generate 5 questions with AI',exact:true}).click();await expect(dialog).toContainText('Question creation is temporarily unavailable');await expect(dialog.getByRole('button',{name:'Confirm and create',exact:true})).toBeDisabled()
 await dialog.getByRole('button',{name:'Cancel',exact:true}).click();await selectOption(dialog.getByRole('combobox',{name:'Number of questions'}),'10');await dialog.getByRole('button',{name:'Generate 10 questions with AI',exact:true}).click();await expect(dialog.getByRole('button',{name:'Confirm and create',exact:true})).toBeDisabled()
...
test('real text intake and source-switch isolation with fixture quote only',async({page})=>{
 ...
 await dialog.getByRole('button',{name:'Generate 5 questions with AI',exact:true}).click();await expect(dialog.getByRole('button',{name:'Confirm and create',exact:true})).toBeVisible()
 ...
 await dialog.getByRole('button',{name:'Cancel',exact:true}).click();await dialog.getByRole('button',{name:'Change creation method'}).click();await dialog.getByRole('button',{name:'PDF or slides to quiz',exact:true}).click();await expect(dialog.getByRole('button',{name:'Generate 5 questions with AI',exact:true})).toBeDisabled()
 await expect(dialog.getByRole('heading',{name:'Upload a file',exact:true})).toBeVisible();await expect(dialog.getByRole('button',{name:/Drop your file here/})).toBeVisible();await page.screenshot({path:`${OUT}/file-source-empty-en-1440.png`})
...
test('real file extraction and unreadable recovery stay source-backed without paid work',async({page})=>{
 ...
 const pageOne=dialog.getByRole('checkbox',{name:'Page 1',exact:true})
 await expect(pageOne).toBeVisible();await expect(pageOne).toBeChecked();await pageOne.uncheck();await expect(pageOne).not.toBeChecked();await pageOne.check();await expect(pageOne).toBeChecked()
...
test('admin configuration renders and saves through existing page (browser DTO fixture)',async({page})=>{
 ...
 await page.goto('/admin/ai-settings');await expect(page.getByRole('heading',{name:'AI routes',exact:true})).toBeVisible()
...
test('Arabic file intake and unavailable message remain readable on mobile',async({page})=>{
 ...
 await expect(dialog).toContainText('ar-photosynthesis.docx');await dialog.getByRole('button',{name:/توليد [5٥] أسئلة بالذكاء الاصطناعي/,exact:true}).click();await expect(dialog).toContainText('إنشاء الأسئلة غير متاح مؤقتًا',{timeout:15000});expect(await dialog.evaluate(el=>el.scrollWidth<=el.clientWidth)).toBe(true)
...
test('file extraction options and editable single-question review',async({page})=>{
 ...
 await expect(d.getByRole('radio',{name:/^Extract existing questions/})).toBeVisible()
 await d.getByRole('radio',{name:/^Extract existing questions/}).check()
 ...
 await generate.click();await d.getByRole('button',{name:'Confirm and create',exact:true}).click();const first=d.getByRole('article',{name:'Question 1',exact:true});await expect(first).toBeVisible()
