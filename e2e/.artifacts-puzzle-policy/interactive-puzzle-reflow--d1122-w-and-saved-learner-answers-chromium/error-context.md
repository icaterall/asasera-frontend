...
- Name: interactive-puzzle-reflow.spec.ts >> edited Arabic passage and ignore policy keep hamza distinct in preview and saved learner answers
  ...
  - main [ref=f1e13]:
    - generic [ref=f1e14]:
      ...
      - region "النص والفراغات" [ref=f1e26]:
        ...
        - group [ref=f1e48]:
          - generic "قواعد الإملاء" [ref=f1e49] [cursor=pointer]
          - generic [ref=f1e50]:
            - text: لغة الكلمات
          ...
          - generic [ref=f1e57]:
            - checkbox "تجاهل الحركات عند المطابقة" [checked] [active] [ref=f1e58]
...
> 126 |  const published=page.waitForResponse(response=>response.url().endsWith(`/activities/${activityId}/publish`)&&response.request().method()==='POST')
      |                       ^ Error: page.waitForResponse: Test timeout of 60000ms exceeded.
  127 |  await page.getByRole('button',{name:'اعتماد التغييرات',exact:true}).click();expect((await published).ok()).toBe(true)
