...
for(const [width,ar] of [[1440,false],[390,true]] as const)test(`image upload progress and preserved edits ${width}`,async({page})=>{
 ...
 const upload=answer.locator('[data-image-upload]'),progress=answer.getByRole('progressbar',{name:ar?'رفع الصورة':'Image upload'}),toolbarButton=answer.getByRole('button',{name:ar?'صورة الإجابة':'Answer image',exact:true})
 await expect(progress).toHaveAttribute('data-phase','uploading');await expect(toolbarButton).toBeDisabled();await expect(toolbarButton).toHaveAttribute('aria-busy','true')
 ...
 releaseUpload();await expect(progress).toHaveAttribute('data-phase','checking')
 await page.screenshot({path:`../screenshots/image-checking-${width}.png`})
 complete=true;await expect(progress).toHaveAttribute('data-phase','ready',{timeout:10_000})
 await expect(text).toHaveText('H2O edited during upload');await expect(toolbarButton).toBeEnabled();await expect(toolbarButton).toHaveAttribute('aria-busy','false');await expect(answer.locator('img')).toBeVisible()
 ...
 await expect(answer.getByRole('alert')).toContainText(ar?'رفع الصور غير متاح مؤقتًا':'Image uploads are temporarily unavailable')
