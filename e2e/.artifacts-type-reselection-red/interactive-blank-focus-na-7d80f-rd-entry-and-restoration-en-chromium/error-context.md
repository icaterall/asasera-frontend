...
- Name: interactive-blank-focus.spec.ts >> native question type dialog preserves keyboard entry and restoration en
...
Locator:  getByRole('textbox', { name: 'Accepted answers for blank 1', exact: true })
Expected: "book"
Received: "water"
Timeout:  5000ms

Call log:
  - Expect "toHaveValue" getByRole('textbox', { name: 'Accepted answers for blank 1', exact: true }) with timeout 5000ms
  - waiting for getByRole('textbox', { name: 'Accepted answers for blank 1', exact: true })
    14 × locator resolved to <textarea rows="2" dir="auto" maxlength="2407">water</textarea>
       - unexpected value "water"

```

```yaml
- textbox "Accepted answers for blank 1": water
```
