...
for(const [lang,width] of [['en',1440],['ar',390]] as const){
 test(`new quiz entry choices and topic ${lang} ${width}`,async({page})=>{
  ...
  let paid=0;await page.route('**/api/v1/activity-generation/**',r=>{if(r.request().url().endsWith('/quote'))return r.fulfill({json:{quoteId:'11111111-1111-4111-8111-111111111111',quoteExpiresAt:'2999-01-01',estimateMillicents:10,maxAuthorizedMillicents:100,estimateAiCredits:10,maxAuthorizedAiCredits:100,usableAiCredits:100000,creditPolicyVersion:1,creditUnit:'AI Credits',spendableMillicents:100000,usableMillicents:100000,allowanceMillicents:100000,exposureMillicents:0,affordable:true,generationAvailable:true,pricingAvailable:true,grant:{trialMillicents:100000,trialAiCredits:100000,claimed:true,eligible:true,reason:null},delivery:'5 questions'}});if(r.request().method()==='POST')paid++;return r.fulfill({json:{jobs:[]}})})
...
test('new quiz file path combines PDF and slides; blank canvas stays manual',async({page})=>{
 ...
 await page.screenshot({path:`${out}/upload-en.png`});await d.locator('input[type=file]').setInputFiles('../asasera-backend/tests/fixtures/en-water-cycle.pdf');await expect(d).toContainText('en-water-cycle.pdf');await expect(d.getByRole('button',{name:'Change source',exact:true})).toBeVisible();await d.getByRole('button',{name:'Change source',exact:true}).click();await d.locator('input[type=file]').setInputFiles('../asasera-backend/tests/fixtures/en-water-cycle.pptx');await expect(d).toContainText('en-water-cycle.pptx');await expect(d.getByRole('checkbox',{name:'Slide 1',exact:true})).toBeAttached()
