...
test('T001 T002 homepage simple quiz and existing standalone/live wheel entrances return correctly',async({page,request},info)=>{
 ...
 const approvedDialog=page.getByRole('dialog',{name:'Your activity is approved',exact:true})
 await expect(approvedDialog).toBeVisible()
 await approvedDialog.getByRole('button',{name:'Keep editing',exact:true}).click()
