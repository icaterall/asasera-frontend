...
async function account(page:Page,role:'teacher'|'student'){
  ...
  await page.locator(`a[data-role="${role}"]`).click()
  ...
  if(role==='teacher')await page.locator('.choice-card').first().click()
    ...
    await selectOption(page.getByLabel('Education stage',{exact:true}),String(stageId))
...
test('teacher registers, follows the actual verification email, creates and publishes from the workspace',async({page})=>{
  ...
  await page.getByRole('link',{name:'Create activity',exact:true}).first().click()
  await page.getByLabel('Activity name',{exact:true}).fill('Learning journey — synthetic activity')
  ...
  for(const [i,answer]of ['Four','Three','Five','Six'].entries())await page.getByLabel(`Answer ${i+1} text`,{exact:true}).fill(answer)
  await page.getByRole('button',{name:'Approve version',exact:true}).click()
  const approved=page.getByRole('dialog',{name:'Your activity is approved',exact:true})
  await expect(approved).toBeVisible()
  await approved.getByRole('button',{name:'Keep editing',exact:true}).click()
...
test('the API applies the same eight-character registration floor shown in the browser',async({page})=>{
  const response=await page.request.post('/api/v1/auth/register/student',{data:{email:identity(),password:'short7!'}})
...
test('student destination handles bad links and Arabic PINs, role protection, refresh and logout',async({page})=>{
  ...
  await expect(page.getByText('Enter the six-digit PIN.',{exact:true})).toBeVisible()
  await page.getByRole('link',{name:'My activities',exact:true}).click()
  await page.getByLabel('Add an activity link',{exact:true}).fill('https://example.com/learn/12345678-1234-1234-1234-123456789012#fake')
  await page.getByRole('button',{name:'Save & open',exact:true}).click()
  await expect(page.getByText('Paste the full Asasera activity link from your teacher.',{exact:true})).toBeVisible()
  await page.getByRole('link',{name:'Home',exact:true}).click()
  await page.getByLabel('Game PIN',{exact:true}).fill('١۲٣۴٥۶')
  await page.getByRole('button',{name:/^Join(?: now| class)?$/,exact:true}).click()
...
test('student profile changes persist and verification resend gives its server cooldown',async({page})=>{
 ...
 await page.getByRole('link',{name:'Learning preferences',exact:true}).click()
 const stages=(await(await page.request.get('/api/v1/education-stages')).json()).stages
 await selectOption(page.getByLabel('Education stage',{exact:true}),String(stages.at(-1).id))
 await page.getByRole('button',{name:'Save preferences',exact:true}).click()
 await expect(page.getByText('Learning preferences saved.',{exact:true})).toBeVisible()
 ...
 await page.reload()
 await page.getByRole('link',{name:'Learning preferences',exact:true}).click()
 await expect(page.getByLabel('Education stage',{exact:true})).toHaveAttribute('data-select-value',String(stages.at(-1).id))
})
