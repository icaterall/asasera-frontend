...
for(const mode of ['class-competition','question-wheel'] as const)test(`actual shared ${mode}: phone rules and teams, two learners, projector and separated report`,async({page,request,browser})=>{
 ...
 await page.getByRole('checkbox',{name:'فكّروا معًا: إضافة 15 ثانية للجميع، مرة لكل سؤال'}).check()
 await expect(page.getByRole('checkbox',{name:'نقاط سرعة معلنة'})).not.toBeChecked()
 await page.getByText('جولات النقاط الإضافية (اختياري)',{exact:true}).click()
 await page.getByRole('spinbutton',{name:'نقاط إضافية للإجابة الصحيحة'}).fill('50')
