...
Locator:  locator('aside [data-question-kind]').first()
Expected: "hotspot"
Received: "mcq"
Timeout:  20000ms

Call log:
  - Expect "toHaveAttribute" locator('aside [data-question-kind]').first() with timeout 20000ms
  - waiting for locator('aside [data-question-kind]').first()
    44 × locator resolved to <button type="button" id="question-kind" aria-expanded="false" aria-haspopup="dialog" aria-label="نوع السؤال" data-question-kind="mcq" class="_trigger_xl664_1">…</button>
       - unexpected value "mcq"
...
```ts
  ...
  20 |   const type=page.locator('aside [data-question-kind]').first()
  ...
  32 |   /* A pin question cannot be stored without a picture. The new-question flow
  33 |      therefore opens its image-first canvas and commits the hotspot kind only
  34 |      after the image is approved. */
  35 |   await expect(page.getByRole('heading',{name:'ابدأ بصورة',exact:true})).toBeVisible()
  36 |   await expect(type).toHaveAttribute('data-question-kind','mcq')
  37 |   const image=await sharp({create:{width:640,height:400,channels:3,background:'#8bd3c7'}}).png().toBuffer()
  38 |   const picker=page.waitForEvent('filechooser');await page.getByRole('button',{name:'ارفع صورة',exact:true}).click();await (await picker).setFiles({name:'synthetic-zones.png',mimeType:'image/png',buffer:image})
> 39 |   await expect(type).toHaveAttribute('data-question-kind','hotspot',{timeout:20_000})
     |                      ^ Error: expect(locator).toHaveAttribute(expected) failed
