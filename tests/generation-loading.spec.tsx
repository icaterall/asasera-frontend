...
it('uses the saved custom activity language over the source, interface and recovered generation choice',async()=>{
 ...
 await waitFor(()=>expect(vi.mocked(api.post).mock.calls.some(([path])=>path.endsWith('/quote'))).toBe(true))
...
it('starts a mode quote immediately and blocks the modal with its logo until the quote finishes',async()=>{
 show({origin:'file',materialRevisionId:11})
 await waitFor(()=>expect(vi.mocked(api.post).mock.calls.filter(([path])=>path.endsWith('/quote'))).toHaveLength(1))
 const original=vi.mocked(api.post).getMockImplementation()!
 let resolveQuote!:(value:typeof quote)=>void
 vi.mocked(api.post).mockImplementation((path,body)=>path.endsWith('/quote')?new Promise(resolve=>{resolveQuote=resolve}):original(path,body))
 const before=vi.mocked(api.post).mock.calls.filter(([path])=>path.endsWith('/quote')).length
 vi.useFakeTimers()
 fireEvent.click(screen.getByRole('radio',{name:/^Extract existing questions/}))
 await act(async()=>{await vi.advanceTimersByTimeAsync(20)})
 expect(vi.mocked(api.post).mock.calls.filter(([path])=>path.endsWith('/quote'))).toHaveLength(before+1)
 const dialog=screen.getByRole('dialog')
 expect(dialog.querySelector('header')?.hasAttribute('inert')).toBe(true)
 expect(screen.getByRole('checkbox',{name:'Page 1'}).closest('[inert]')).not.toBeNull()
 expect(dialog.querySelector('footer')?.hasAttribute('inert')).toBe(true)
 expect(screen.getByRole('status').querySelector('img')?.getAttribute('src')).toContain('logo-box-loader')
 await act(async()=>{resolveQuote(quote)})
 expect(screen.queryByRole('status')).toBeNull()
 expect(screen.getByRole('checkbox',{name:'Page 1'}).closest('[inert]')).toBeNull()
 expect((screen.getByRole('button',{name:'Extract questions with AI'}) as HTMLButtonElement).disabled).toBe(false)
})
it('unlocks the modal after a quote fails without enabling generation against the previous price',async()=>{
 show({origin:'file',materialRevisionId:11})
 const generate=await screen.findByRole('button',{name:'Generate 5 questions with AI'})
 await waitFor(()=>expect((generate as HTMLButtonElement).disabled).toBe(false))
 let rejectQuote!:(reason:Error)=>void
 vi.mocked(api.post).mockImplementation(()=>new Promise((_resolve,reject)=>{rejectQuote=reject}))
 fireEvent.click(screen.getByRole('radio',{name:/^Extract existing questions/}))
 await screen.findByRole('status')
 await act(async()=>{rejectQuote(new Error('Connection interrupted'))})
 expect(screen.queryByRole('status')).toBeNull()
 expect(screen.getByRole('checkbox',{name:'Page 1'}).closest('[inert]')).toBeNull()
 expect((screen.getByRole('button',{name:'Extract questions with AI'}) as HTMLButtonElement).disabled).toBe(true)
 expect(screen.getByText('This request could not be completed. Please try again.')).toBeTruthy()
})
it('cancels an estimate request when the modal is closed',async()=>{
 let signal:AbortSignal|undefined
 vi.mocked(api.post).mockImplementation((_path,_body,options)=>{signal=options?.signal;return new Promise(()=>{})})
...
it('blocks recent-result navigation while a submission outcome is unknown',async()=>{const get=vi.mocked(api.get).getMockImplementation()!,post=vi.mocked(api.post).getMockImplementation()!;vi.mocked(api.get).mockImplementation(p=>p.includes('/activity-generation/activities/')?Promise.resolve({jobs:[{...job,id:91,state:'succeeded'}]}):get(p));vi.mocked(api.post).mockImplementation((p,b)=>p.endsWith('/jobs')?Promise.reject(new Error('lost response')):post(p,b));show();const b=await topic();await waitFor(()=>expect((b as HTMLButtonElement).disabled).toBe(false));fireEvent.click(b);await screen.findByText(/This request is awaiting confirmation/);const recent=screen.queryByRole('button',{name:'Resume questions 1',hidden:true}) as HTMLButtonElement|null;if(recent){expect(recent.disabled).toBe(true);fireEvent.click(recent)}expect(screen.getByLabelText('Lesson topic')).toBeTruthy();expect(vi.mocked(api.get).mock.calls.some(([p])=>p.endsWith('/jobs/91'))).toBe(false)})
