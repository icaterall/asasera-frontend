...
export function InboxEntry({ item, onSaved, onReview, onRefresh }: { item: InboxItem; onSaved: () => void; onReview?: ReviewInEditor; onRefresh?: () => void }) {
  ...
  const reasons: Record<string, string> = { comment: t('ملاحظة على السؤال', 'Question comment'), incorrect_answer: t('إجابة تحتاج مراجعة', 'Check the answer'), unclear_wording: t('صياغة غير واضحة', 'Unclear wording'), inappropriate_content: t('محتوى غير مناسب', 'Inappropriate content'), technical_problem: t('مشكلة تقنية', 'Technical problem') }
  ...
  return <article className={styles.inboxEntry}>
    ...
    <p className={styles.muted}><bdi>{item.reviewerRole==='student'?t('طالب', 'Student'):item.reviewerName}</bdi>{item.reviewerRole==='teacher'&&` · ${t('معلّم', 'Instructor')}`} · <time dateTime={item.updatedAt}>{new Date(item.updatedAt).toLocaleDateString(ar ? 'ar' : 'en')}</time>{item.rating && ` · ${item.rating} / 5`}{item.recommend && ` · ${t('يوصي بالنشاط', 'Recommends this activity')}`}</p>
    ...
    <details className={styles.respond}><summary>{t('الردّ وتحديث الحالة', 'Reply and update status')}</summary>
      <form onSubmit={e => { e.preventDefault(); if (!save.isPending) save.mutate(draft.value) }}>
        <label className={styles.field}>{t('ردّك لصاحب الملاحظة (اختياري)', 'Your reply to the reviewer (optional)')}<textarea dir="auto" rows={3} value={response} maxLength={2000} disabled={save.isPending} onChange={e => setResponse(e.target.value)} /></label>
...
export default function FeedbackInbox() {
  ...
  return <div className={`asas ${styles.page}`}>
    <header className={styles.heading}><div className={styles.titleGroup}><MessageSquare size={36} aria-hidden="true" /><div><h1>{t('الملاحظات وأفكار التحسين', 'Feedback & ideas')}</h1><p>{t('استمع للمعلّمين والطلاب، وراجع ملاحظاتهم على نشاطك.', 'Read instructor and student reviews, reply, and improve your activity.')}</p></div></div><Link to="/teacher/activities">{t('أنشطتي', 'My activities')}</Link></header>
    ...
    <div className={activityId ? styles.inboxLayout : ''}>
      <section aria-label={t('ملاحظات المعلّمين والطلاب', 'Instructor and student feedback')}>
