...
function SharedActivityContent({ activityId }: { activityId: number }) {
  ...
  const context = useQuery({ queryKey: ['shared-activity-context', activityId, user?.id], queryFn: () => community.context(activityId), enabled: !!data.data && (user?.role === 'teacher' || user?.role === 'student') })
  ...
  return <div className={`asas ${styles.page}`}>
    {stopped ? <section className={styles.empty}><Lock size={40} aria-hidden="true" /><h1>{t('أصبح النشاط خاصًا', 'Your activity is now private')}</h1><p>{t('توقّف رابط المشاركة عن عرض النشاط. يمكنك إعادة نشره من المحرّر.', 'The shared link no longer displays this activity. You can publish it again from the editor.')}</p><Link to={`/teacher/activities/${activityId}`}>{t('فتح المحرّر', 'Open editor')}</Link></section> : data.isPending ? <LoadingState label={t('جارٍ فتح النشاط', 'Opening activity')} /> : data.error || !a ? <FailureState title={t('النشاط غير متاح', 'This activity isn’t available')} body={t('قد يكون صاحبه جعله خاصًا أو أزاله.', 'Its creator may have made it private or removed it.')} actions={<><Button onClick={() => void data.refetch()}>{t('إعادة المحاولة', 'Try again')}</Button><Link to={user?.role === 'teacher' ? '/teacher/discover' : '/'}>{t('العودة', 'Go back')}</Link></>} /> : <>
      ...
      <p className={styles.ownership}>{t('نشاط عام · يبقى ملكًا لصاحبه. يمكن مشاركته وتقييمه، ولا يستطيع غير صاحبه تعديل الأصل.', 'Public activity · Owned by its creator. You can share and review it; only the creator can edit the original.')}</p>
      <div className={styles.activityLayout}>
        ...
        {question && user?.role === 'teacher' && context.data && !context.data.isAuthor && <QuestionComment key={`${question.id}-${context.data.flags.find(f=>f.questionId===question.id&&f.reason==='comment')?.note??''}`} activityId={activityId} questionId={question.id} versionId={a.versionId} initial={context.data.flags.find(f=>f.questionId===question.id&&f.reason==='comment')?.note} onSaved={saved}/>}
        </section>
        <aside className={styles.sharing}>
          ...
          <div className={styles.socialProof}>{a.summary.reviewCount ? <><span><Star size={18} aria-hidden="true" />{a.summary.averageRating} / 5 · {a.summary.reviewCount} {t('تقييمات معلّمين', 'instructor reviews')}</span><span><ThumbsUp size={18} aria-hidden="true" />{a.summary.recommendationCount} {t('توصيات من معلّمين', 'teacher recommendations')}</span></> : <p>{t('لم يتلقَّ تقييمًا من المعلّمين بعد.', 'No teacher reviews yet.')}</p>}{a.summary.studentReviewCount ? <span><Star size={18} aria-hidden="true" />{a.summary.studentAverageRating} / 5 · {a.summary.studentReviewCount} {t('تقييمات طلاب', 'student reviews')}</span> : <p>{t('لا توجد تقييمات طلاب بعد.', 'No student reviews yet.')}</p>}</div>
      ...
      {(user?.role === 'teacher' || user?.role === 'student') && (context.isPending ? <LoadingState rows={1} /> : context.error ? <FailureState title={t('تعذّر تحميل ملاحظاتك', 'Your feedback couldn’t load')} actions={<Button onClick={() => void context.refetch()}>{t('إعادة المحاولة', 'Try again')}</Button>} /> : context.data && !context.data.isAuthor ? <FeedbackForm key={`${activityId}-${user.id}-${context.data.feedback?.revision ?? 0}`} activityId={activityId} versionId={a.versionId} student={user.role === 'student'} initial={context.data.feedback} onSaved={saved} /> : <p className={styles.notice}>{t('هذا نشاطك. شارك الرابط مع المعلّمين، ثم تابع ملاحظاتهم في صندوق الملاحظات.', 'This is your activity. Share the link with other teachers, then follow their suggestions in your feedback inbox.')}</p>)}
      {!!context.data?.flags.length && <section className={styles.feedbackForm}><h2>{t('ملاحظاتك وبلاغاتك عن الأسئلة', 'Your question comments and reports')}</h2>{context.data.flags.map(flag => <div className={styles.reply} key={flag.id}><h3>{t('السؤال', 'Question')} {a.questions.findIndex(q=>q.id===flag.questionId)>=0?a.questions.findIndex(q=>q.id===flag.questionId)+1:t('من نسخة سابقة', 'from an earlier version')}</h3><p>{({open:t('قيد المراجعة','Awaiting review'),reviewed:t('تمت المراجعة','Reviewed'),resolved:t('تمت المعالجة','Resolved')})[flag.status]}</p>{flag.note && <p dir="auto">{flag.note}</p>}{flag.response && <><h3>{t('ردّ صاحب النشاط', 'The creator’s reply')}</h3><p dir="auto">{flag.response}</p></>}</div>)}</section>}
      {user?.role === 'teacher' && <Recommendations activityId={activityId} />}
    </>}
  </div>
}
