...
export function ZoneSuggestions({activityId,question,imageUrl,onPrepare,onApplied,onClose}:{activityId:number;question:QuestionRecord;imageUrl:string|null;onPrepare:()=>Promise<QuestionRecord>;onApplied:()=>Promise<void>;onClose:()=>void}){
 ...
 return createPortal(<dialog ref={dialog} className={`asas ${styles.suggestionsDialog}`} dir={ar?'rtl':'ltr'} aria-labelledby={title} onCancel={e=>{e.preventDefault();if(pending!=='apply')onClose()}} onKeyDown={e=>e.stopPropagation()}>
  <header className={styles.dialogHeader}><div><h2 id={title}>{t('اقترح مناطق الإجابة','Suggest answer areas')}</h2><p>{t('يقرأ الذكاء الاصطناعي الصورة والسؤال ليقترح الإجابات الصحيحة وأماكنها، حتى دون مناطق موجودة. راجع الاقتراحات قبل إضافتها.','AI reads the image and question to suggest correct answers and their areas, even when starting empty. Review the suggestions before adding them.')}</p></div><Button variant="quiet" aria-label={t('إغلاق','Close')} disabled={pending==='apply'} onClick={onClose}><X size={20}/></Button></header>
  <div className={styles.suggestionsBody} inert={pending==='quote'||pending==='apply'}>
   {!jobId&&<p>{t('حساب التكلفة مجاني: لا يستدعي الذكاء الاصطناعي ولا يستهلك رموزًا أو رصيدًا. التوليد فقط يستهلك رصيد الذكاء الاصطناعي.','Estimating is free: no AI call, tokens, or AI Credits are used. Only generation uses AI Credits.')}</p>}
   ...
   {/* A sentence alone cannot say whether anything is still happening. The
       indicator is inline rather than an overlay on purpose: the teacher may
       close this panel and come back to the same request, and an overlay would
       take that away exactly when the wait is longest. */}
   {preparing&&<div role="status" className={styles.suggestionsWaiting}>
     <LoadingIndicator label={t('جارٍ تحديد الأجزاء المرئية وتسمياتها…','Finding visible parts and their labels…')}/>
     <p>{t('يمكنك إغلاق النافذة والعودة إلى الطلب نفسه.','You can close this panel and return to the same request.')}</p>
   </div>}
