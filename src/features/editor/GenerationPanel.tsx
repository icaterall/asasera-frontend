...
type Quote={quoteId:string;quoteExpiresAt:string;estimateMillicents:number;maxAuthorizedMillicents:number;estimateAiCredits:number;maxAuthorizedAiCredits:number;usableAiCredits:number;creditPolicyVersion:number;creditUnit:'AI Credits';usableMillicents:number;affordable:boolean;generationAvailable:boolean;pricingAvailable:boolean}
...
/**
 * One page of the source, drawn.
 *
 * Choosing pages out of a coursebook is something a teacher does BY EYE — they
 * recognise the spread with the vocabulary box on sight, and cannot recognise
 * it from the first lines of its extracted text. The picker used to show that
 * text, which is the one representation of a page nobody reads.
 *
 * `loading="lazy"` is the whole scheduling strategy: the server rasterises on
 * request, so a 400-page book costs nothing until a row is scrolled into view.
 *
 * Falls back to the text when there is no picture to draw — a pasted-text
...
export function GenerationPanel({startWithChoices=false,activity,question,onClose,onApplied,initialDraft=null,replacement=false,provenance=null}:{startWithChoices?:boolean;activity:ActivityRecord;question:QuestionRecord|null;onClose:()=>void;onApplied:()=>Promise<void>;initialDraft?:GenerationDraft|null;replacement?:boolean;provenance?:QuestionProvenance|null}){
 ...
 const previewToken=pages.data?.previewToken??''
  ...
  <div className={styles.body} inert={quoting}>
   ...
   {!jobId&&<fieldset className={`${styles.form} ${!replacement?styles.creationForm:''}`} disabled={!!pending||!!submitted}>
    ...
    {/* Two real choices, each carrying its own consequence. The description
        ...
        NOT picked never explained itself — the reading of "extract" mattering
        most to a teacher who has not tried it. */}
    {!replacement&&fileOrigin&&revisionId&&<fieldset className={styles.extractionMode}>
     <legend>{ar?'كيف تريد إنشاء الأسئلة؟':'How should we create questions?'}</legend>
     {([
      ['generate',ar?'توليد أسئلة جديدة':'Generate new questions',ar?'إنشاء أسئلة جديدة بناءً على الصفحات المحددة.':'Create new questions based on the selected pages.'],
      ['extract',ar?'استخراج الأسئلة الموجودة':'Extract existing questions',ar?'نسخ الأسئلة الموجودة في الصفحات المحددة دون اختراع أسئلة أو إجابات جديدة. تُحفظ لغة النص الأصلية.':'Copy questions already in the selected pages, without inventing any. The original wording is kept.'],
     ] as const).map(([value,label,detail])=>
      <label key={value} className={styles.modeOption}>
       <input type="radio" name="creation-mode" checked={mode===value} onChange={()=>setMode(value)}/>
       <strong>{label}</strong>
       <small>{detail}</small>
      </label>)}
    </fieldset>}
    ...
    {fileOrigin&&revisionId&&<section className={styles.fileSummary}><strong><FileText size={18} aria-hidden="true"/> {title}</strong>{revision.isPending||['pending','running'].includes(revision.data?.revision.state??'')?<LoadingIndicator label={t('processing')}/>:revision.isError?<p role="alert">{t('failed')}</p>:revision.data?.revision.state==='failed'?<p role="alert">{t('noText')}</p>:<>
     ...
     {!replacement&&<>
      <div className={styles.pageTools}>
       {caps&&<span className={styles.pageLimit}>{ar?`الحد: ${number(caps.maxSegmentsPerGeneration)} صفحة`:`Page limit: ${number(caps.maxSegmentsPerGeneration)} pages`}</span>}
       <Button variant="quiet" onClick={()=>setSegments(segments.length===readable.length?[]:readable.map(s=>s.segmentIndex))}>{segments.length===readable.length?t('deselect'):t('selectAll')}</Button>
      </div>
      <div className={styles.pagePreviews}>{readable.map(s=>{
