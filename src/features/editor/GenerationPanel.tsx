...
export function GenerationPanel({startWithChoices=false,activity,question,onClose,onApplied,initialDraft=null,replacement=false,provenance=null}:{startWithChoices?:boolean;activity:ActivityRecord;question:QuestionRecord|null;onClose:()=>void;onApplied:()=>Promise<void>;initialDraft?:GenerationDraft|null;replacement?:boolean;provenance?:QuestionProvenance|null}){
 ...
 const [language]=useState(()=>saved?.language??activity.contentLanguage??initial.language??defaultContentLanguage(i18n.language))
 ...
 return <dialog ref={dialog} aria-labelledby={titleId} dir={ar?'rtl':'ltr'} className={`asas ${styles.dialog} ${!replacement?(choosing?styles.chooser:styles.workspace):''} ${!choosing&&!replacement&&!jobId?styles.creationWorkspace:''} ${origin==='file'?styles.fileWorkspace:styles.topicWorkspace}`} onCancel={event=>{event.preventDefault();event.stopPropagation();onClose()}} onKeyDown={event=>{if(event.key==='Escape'){event.stopPropagation();return}if(event.key!=='Tab')return;const items=Array.from(dialog.current?.querySelectorAll<HTMLElement>('button:not(:disabled),a[href],input:not(:disabled),textarea:not(:disabled),select:not(:disabled),summary,[tabindex="0"]')??[]).filter(el=>el.tabIndex>=0&&el.getClientRects().length>0&&!el.closest('[hidden],[inert]'));const first=items[0],last=items.at(-1);if(!first)return;if(event.shiftKey&&(document.activeElement===first||!dialog.current?.contains(document.activeElement))){event.preventDefault();last?.focus()}else if(!event.shiftKey&&(document.activeElement===last||!dialog.current?.contains(document.activeElement))){event.preventDefault();first.focus()}}}>
  ...
  <div className={styles.body}>
   ...
   {!jobId&&<fieldset className={`${styles.form} ${!replacement?styles.creationForm:''}`} disabled={!!pending||!!submitted}>
    ...
    {!replacement&&(origin!=='file'||revisionId)&&<details className={styles.settings} open><summary><span>{contentLanguageName(language,i18n.language)} · {t(difficulty)} · {t(kinds.length===2?'mixed':kinds[0]??'mixed')}</span><strong>{t('settings')}</strong></summary><div className={styles.settingsFields}><label>{t('difficulty')}<Select disabled={!!pending||!!submitted||mode==='extract'} value={difficulty} onValueChange={v=>{setDifficulty(v as Recovery['difficulty']);setSubmitted(undefined)}}>{['easy','medium','hard'].map(v=><option key={v} value={v}>{t(v)}</option>)}</Select></label>{/* Shown, not chosen. The server decides this from the activity:
        generation.service.ts normalize() takes activity.contentLanguage and
        ignores the request's language, so an editable control here spent the
        teacher's credit and returned the other language without a word. It is
        also the same decision already made on the create form. Read-only keeps
        the teacher informed and removes a choice that never worked. */}
