...
export default function ActivityList() {
  ...
  /* v5: approval and public sharing are different states. Draft = no approved
     version yet; Approved = runnable privately; Shared = also in the library. */
  const matches = (a: { visibility: string; currentVersionId: number | null }) =>
    status === 'all' || (status === 'draft' && !a.currentVersionId) || (status === 'approved' && !!a.currentVersionId && a.visibility !== 'published') || (status === 'shared' && a.visibility === 'published')
  const items = data.data?.activities.filter(a => matches(a) && a.title.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())) ?? []
  ...
  return <div className={`asas ${styles.page}`}>
    ...
    <div className={styles.search}>
      ...
      <label className={styles.field}>{ar ? 'الحالة' : 'Status'}<Select value={status} onValueChange={e => setStatus(e)}><option value="all">{ar ? 'الكل' : 'All activities'}</option><option value="draft">{ar ? 'مسودات' : 'Drafts'}</option><option value="approved">{ar ? 'معتمدة' : 'Approved'}</option><option value="shared">{ar ? 'مشاركة في المكتبة' : 'Shared to library'}</option></Select></label>
    </div>
