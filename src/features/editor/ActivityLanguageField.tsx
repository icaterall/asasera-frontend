 ...
 */
export function ActivityLanguageField({value,onChange,disabled=false,hint=true}:{value:string;onChange:(value:string)=>void;disabled?:boolean;hint?:boolean}){
 ...
 return <div className={styles.field}>
  ...
  <Select id={id} value={known?value:customValue} disabled={disabled} aria-describedby={hint?`${id}-hint`:undefined} onValueChange={next=>onChange(next==='__other__'?'':next)}>
  ...
  {hint&&<p id={`${id}-hint`}>{ar?'تُحفظ مع النشاط وتُستخدم لتوليد المحتوى بالذكاء الاصطناعي. تغيير لغة الواجهة لا يغيّرها.':'Saved with this activity and used for AI-generated content. Changing the interface language won’t change this choice.'}</p>}
 </div>
}
