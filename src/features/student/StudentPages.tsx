...
function ProfileForm({ initial }: { initial: LearningProfile }) {
  ...
  async function submit(e: FormEvent) { e.preventDefault(); if (inFlight.current) return; inFlight.current = true; setBusy(true); setError(''); setSaved(false)
    try { const result = await studentApi.profile(profile); client.setQueryData(studentQueryKey(user!.id), { ...data, profile: result.profile }); setSaved(true) } catch (e) { setError(e instanceof Error ? e.message : t('تعذّر الحفظ.', 'Could not save.')) } finally { setBusy(false); inFlight.current = false }
  }
  return <form className={`${styles.profileForm} ${styles.profilePanel}`} onSubmit={submit}><StageGradePicker value={profile} onChange={next => { setProfile(next); setSaved(false) }} ar={ar} disabled={busy} /><p className={styles.hint}>{t('تؤثّر هذه التفضيلات في مساحة تعلّمك وأسئلة التدريب. تظل أنشطة المعلّم كما اختارها لك.', 'These preferences adjust your learning space and practice questions. Teacher activities use the questions your teacher chose.')} </p>{error && <p role="alert" className={styles.error}>{error}</p>}<div className={styles.formActions}><button className={styles.primary} disabled={busy}>{busy ? t('جارٍ الحفظ…', 'Saving…') : t('احفظ التفضيلات', 'Save preferences')}</button><Link to="/student">{t('العودة إلى الرئيسية', 'Back to home')}</Link></div>{saved && <p className={styles.success} role="status"><Check size={20} />{t('تم حفظ تفضيلات التعلّم.', 'Learning preferences saved.')}</p>}</form>
}
