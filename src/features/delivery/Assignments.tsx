...
type Assignment={id:string;mode:string;title:string;deadline:string;closedAt:string|null;runId:number;attempts:number;submitted:number;versionId:number}
export default function Assignments(){
 ...
 return <main className={`asas ${styles.setup}`}><header><h1>{t('الواجبات والتعلّم الذاتي','Homework and self-study')}</h1><p>{t('تابع التسليمات، وشارك الرابط أو أغلق النشاط لعرض تقريره.','Track submissions, share a link, or close an assignment to view its report.')}</p><Link to="/teacher/activities">{t('اختر نشاطًا','Choose an activity')}</Link></header>
  ...
  {data.isPending?<LoadingState/>:data.error?<FailureState title={t('تعذّر تحميل الواجبات','Assignments unavailable')} body={data.error.message} actions={<Button onClick={()=>void data.refetch()}>{t('أعد المحاولة','Retry')}</Button>}/>:data.data.assignments.length?data.data.assignments.map(a=><article className={styles.assignment} key={a.id}><h2>{a.title}</h2><p>{a.mode==='study'?t('تعلّم ذاتي','Self-study'):t('واجب','Homework')} · {a.closedAt?t('مغلق','Closed'):t('متاح','Open')} · {new Date(a.deadline).toLocaleString(ar?'ar':'en')}</p><p>{t(`${a.submitted} تسليمات من ${a.attempts} مشاركين`,`${a.submitted} submitted of ${a.attempts} participants`)}</p><div className={styles.actions}>{a.closedAt?<Link to={`/teacher/reports/runs/${a.runId}`}>{t('عرض التقرير','View report')}</Link>:<><Button disabled={busy} onClick={()=>void work(async()=>{const r=await api.get<{id:string;accessToken:string}>(`/api/v1/delivery/assignments/${a.id}/link`),url=assignmentUrl(r.id,r.accessToken);try{await navigator.clipboard.writeText(url);setNotice(t('تم نسخ الرابط','Link copied'))}catch{setManual(url)}})}>{t('انسخ الرابط','Copy link')}</Button><Button disabled={busy} onClick={()=>setClosing(a)}>{t('أغلق النشاط','Close assignment')}</Button></>}</div></article>):<p>{t('ابدأ من نشاط معتمد، ثم اختر واجبًا أو تعلّمًا ذاتيًا.','Open an approved activity, then choose homework or self-study.')}</p>}
