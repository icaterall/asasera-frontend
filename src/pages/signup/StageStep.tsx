...
export default function StageStep() {
  ...
  return <AuthCard title={title} lead={ar ? 'اختر مرحلتك لنجهّز مساحة تعلّم تناسبك، أو اختر تعلّمًا عامًا.' : 'Choose your stage to personalize your learning space, or keep it general.'} width="choice">
    <form className={`asas ${styles.profileForm}`} onSubmit={event => { event.preventDefault(); setLearningProfile(profile); navigate('/signup/student/method') }}>
      <StageGradePicker value={profile} onChange={setProfile} ar={ar} />
      <button className={styles.primary} type="submit">{ar ? 'متابعة' : 'Continue'}</button>
    </form>
  </AuthCard>
}
